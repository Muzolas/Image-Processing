...
# Facebook profil fotoğraflarını toplama
...
def main():
    # Kullanıcı adı ve şifre bilgilerini girdiğimiz yer
    email = "****************"
    password = "**************"
    ...
    # Tarayıcı başlatma ve giriş yapma
    driver = initialize_driver()
    login_facebook(driver, email, password)
    get_profile_pictures(driver, profile_url, save_folder)
    driver.quit()
    
    model = torch.hub.load('ultralytics/yolov5', 'yolov5s')  # YOLOv5 modeli

    # Fotoğraflarda insan tespiti
    ...
    # Sonuçları Txt dosyasına yazdırma işlemi
